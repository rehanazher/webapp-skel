...
import com.aaut.skeleton.commons.util.JdbcUtils;
import com.aaut.skeleton.commons.util.Pagination;
import com.aaut.skeleton.commons.util.StringUtils;
import com.aaut.skeleton.commons.util.UUIDGenerator;
...
public class BasicDao<VO> extends JdbcDaoSupport {
	 ...
	 * @return 结果对象列表
	 */
	protected List<VO> query(String sql, MultiRowMapper<VO> multiRowMapper) {
		if (logger.isDebugEnabled()) {
			logger.debug(sql);
		}
		long startTime = System.currentTimeMillis();
		...
		try {
			return (List<VO>) getJdbcTemplate().query(sql,
					new MultiResultSetExtractor<VO>(multiRowMapper));
		} finally {
	 ...
	 * @return 结果对象列表
	 */
	protected List<VO> query(String sql, Object[] args, int[] argTypes,
			MultiRowMapper<VO> multiRowMapper) {
		if (logger.isDebugEnabled()) {
			logger.debug(JdbcUtils.getSQL(sql, args));
		}
		long startTime = System.currentTimeMillis();
		...
		try {
			return (List<VO>) getJdbcTemplate().query(sql, args, argTypes,
					new MultiResultSetExtractor<VO>(multiRowMapper));
		} finally {
	 ...
	 * @return 结果对象列表
	 */
	protected List<VO> query(final String sql, final Object[] args,
			final int[] argTypes, final MultiRowMapper<VO> multiRowMapper,
		...
		try {
			...
			return (List<VO>) getJdbcTemplate().execute(psCreator, psCallback);
		} finally {
			processOvertime(startTime, sql, args);
		}
	}
	 ...
	 * @return 结果对象
	 */
	protected VO query(String sql, Object[] args, int[] argTypes,
			SingleRowMapper<VO> singleRowMapper) {
		if (logger.isDebugEnabled()) {
			logger.debug(JdbcUtils.getSQL(sql, args));
		}
		long startTime = System.currentTimeMillis();
		...
		try {
			return (VO) getJdbcTemplate().query(sql, args, argTypes,
					new SingleResultSetExtractor<VO>(singleRowMapper));
		} finally {
			processOvertime(startTime, sql, args);
		}
	}
	 ...
	 * @return 结果对象列表
	 */
	protected List<VO> query(String sql, Object[] args,
			MultiRowMapper<VO> multiRowMapper) {
		if (logger.isDebugEnabled()) {
			logger.debug(JdbcUtils.getSQL(sql, args));
		}
		...
		try {
			return (List<VO>) getJdbcTemplate().query(sql, args,
					new MultiResultSetExtractor<VO>(multiRowMapper));
		} finally {
			processOvertime(startTime, sql, args);
		}
	 ...
	 */
	protected List<VO> query(String sql, Object[] args,
			MultiRowMapper<VO> multiRowMapper, Pagination page) {
	 ...
	 * @return 结果对象
	 */
	protected VO query(String sql, Object[] args,
			SingleRowMapper<VO> singleRowMapper) {
		if (logger.isDebugEnabled()) {
			logger.debug(JdbcUtils.getSQL(sql, args));
		}
		...
		try {
			return (VO) getJdbcTemplate().query(sql, args,
					new SingleResultSetExtractor<VO>(singleRowMapper));
		} finally {
			processOvertime(startTime, sql, args);
		}
	 ...
	 * @return 结果对象
	 */
	protected VO query(String sql, SingleRowMapper<VO> singleRowMapper) {
		if (logger.isDebugEnabled()) {
			logger.debug(sql);
		}
		long startTime = System.currentTimeMillis();
		...
		try {
			return (VO) getJdbcTemplate().query(sql,
					new SingleResultSetExtractor<VO>(singleRowMapper));
		} finally {
			processOvertime(startTime, sql, null);
		}
	 ...
	 * @return 结果对象列表
	 */
	protected List<VO> query(String sql, String arg,
			MultiRowMapper<VO> multiRowMapper) {
		if (logger.isDebugEnabled()) {
	 ...
	 * @return 结果对象
	 */
	protected VO query(String sql, String arg,
			SingleRowMapper<VO> singleRowMapper) {
		if (logger.isDebugEnabled()) {
			logger.debug(JdbcUtils.getSQL(sql, new Object[] { arg }));
		}
		...
		try {
			return (VO) getJdbcTemplate().query(sql, new Object[] { arg },
					new SingleResultSetExtractor<VO>(singleRowMapper));
		} finally {
			processOvertime(startTime, sql, new Object[] { arg });
		}
	 ...
	 * @return List
	 */
	protected List<VO> queryForInSQL(String prefix, Object[] prefixArgs,
			Object[] inArgs, MultiRowMapper<VO> multiRowMapper) {
	 ...
	 */
	protected List<VO> queryForInSQL(String prefix, Object[] prefixArgs,
			Object[] inArgs, MultiRowMapper<VO> multiRowMapper, String postfix) {
	...
	private void queryForInSQL(String prefix, Object[] prefixArgs,
		...
		for (int i = 0, length = inArgs.length; i < length; i += count) {
			...
			if (rowMapper instanceof MapRowMapper) {
				...
				((List<VO>) result).addAll(query(sql, args,
						(MultiRowMapper<VO>) rowMapper));
			}
		}
	}
	 ...
	 * @return 结果对象列表
	 */
	protected List<VO> queryForTop(String sql, Object[] args, int[] argTypes,
			MultiRowMapper<VO> multiRowMapper, int topLimit) {
		if (logger.isDebugEnabled()) {
			logger.debug(JdbcUtils.getSQL(sql, args));
		}
		long startTime = System.currentTimeMillis();
		...
		try {
			return (List<VO>) getJdbcTemplate()
					.query(
							sql,
							args,
							argTypes,
							new MultiTopResultSetExtractor<VO>(multiRowMapper,
									topLimit));
		} finally {
			processOvertime(startTime, sql, args);
		}
